<%-- 
    Document   : almacenes
    Created on : 2 mar 2025, 13:55:16
    Author     : nodel
--%>
...
<%
    ...
    try {
        almacenes = dao.findAll();
    } catch (ExcepcionDeAplicacion e) {
        out.print("Error al obtener los almacenes: " + e.getMessage());
    }
%>
        ...
        <% if (almacenes != null && !almacenes.isEmpty()) { %>
            <% for (Almacen almacen : almacenes) { %>
                <div class="card-layout__item">
                    ...
                    <img src="assets-copia/images/warehouse/512px-<%= almacen.getCodigo() %>.jpg" alt="Imagen de <%= almacen.getCiudad() %>" class="almacen-image">

                    <h2><%= almacen.getCiudad() %></h2>

                    <p><%= almacen.getCalle() %>, <%= almacen.getCp() %> (<%= almacen.getProvincia() %>)</p>

                    <a href="almacen_detalle.jsp?codigo=<%= almacen.getCodigo() %>" class="btn-cambiar">Cambiar</a>
                </div>
            <% } %>
        <% } else { %>
